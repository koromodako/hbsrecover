    ...
    def __new__(cls, name, bases, dct):
        ...
        for mandatory_const in MetaHBSEncryptedFile.MANDATORY_CONSTS:
            if not mandatory_const in dct:
                raise RuntimeError(f"Subclasses must define '{mandatory_const}'")
        version = dct['VERSION']
        if version in MetaHBSEncryptedFile.VERSION_MAGIC_MAP:
            raise RuntimeError(f"version ({version}) already defined in metaclass structures.")
...
class BaseHBSEncryptedFile(metaclass=MetaHBSEncryptedFile):
    ...
    def __init__(self, filepath):
        ...
        if not filepath.is_file():
            raise RuntimeError("HBSEncryptedFile.filepath shall point to a regular file!")
    ...
    def decrypt(self, passphrase, outdir):
        ...
        '''
        raise NotImplementedError("HBSEncryptedFile subclasses shall define .decrypt() method")
